  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name              = "shrine-service"
          image             = "${var.registry_url}/shrine-service:latest"
...
resource "kubernetes_service" "shrine_service" {
  ...
  spec {
    selector = {
      app = kubernetes_deployment.shrine_service.spec[0].template[0].metadata[0].labels.app
    }
...
# Shrine Service HPA
resource "kubernetes_horizontal_pod_autoscaler_v2" "shrine_service" {
  metadata {
  ...
  spec {
    ...
    max_replicas = var.shrine_service_replicas
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name              = "location-service"
          image             = "${var.registry_url}/location-service:latest"
          image_pull_policy = "Always"
...
resource "kubernetes_service" "location_service" {
  ...
  spec {
    selector = {
      app = kubernetes_deployment.location_service.spec[0].template[0].metadata[0].labels.app
    }
...
resource "kubernetes_horizontal_pod_autoscaler_v2" "location_service" {
  metadata {
  ...
  spec {
    ...
    max_replicas = var.location_service_replicas
