...
} from '@app/shared/interfaces/location';

@Injectable()
export class ShrineService implements OnModuleInit {
  private locationService: LocationServiceClient | null = null;
    ...
    try {
      ...
      if (data.location && this.locationService) {
        ...
        try {
          ...
          this.logger.error(
            `Location resolution failed for "${data.location}":`,
            err,
          );
        ...
        this.logger.log(
          `location-service lookup for "${data.location}" took ${Date.now() - t0}ms`,
        );
      }
  ...
  async findAllShrines(): Promise<ShrineResponse[]> {
    try {
      ...
      this.logger.log(
        `DB findAllShrines took ${Date.now() - t0}ms; count=${shrines.length}`,
      );

      // Debug: Log first shrine to check imageUrl
      if (shrines.length > 0) {
        this.logger.debug(
          'First shrine raw data:',
          JSON.stringify({
            id: shrines[0].id,
            name: shrines[0].name,
  ...
  async updateShrineById(
    ...
    if (data.location && this.locationService) {
      ...
      try {
        ...
        this.logger.log(
          `location-service lookup for update ${data.location} took ${Date.now() - t0}ms`,
        );
      } catch (err) {
        throw new RpcException({
          code: status.INVALID_ARGUMENT,
